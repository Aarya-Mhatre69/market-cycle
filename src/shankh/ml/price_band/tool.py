...
LangChain Tools for live Quantile Regression Price-Band inference (LightGBM & XGBoost).

Flow
----
1. Fetch OHLCV history from yfinance (enough bars to warm up all features).
2. Run add_features() to build the full feature frame.
3. Load trained upper / lower LightGBM or XGBoost models + feature_cols from disk.
4. Call predict_next_day_band() on the target row.
5. Return a JSON summary — no parquet lookup, no pre-computed predictions.
"""
...
def query_gbm_price_band(
    ...
    """
    Predict the next-day high/low price band for an NSE stock using the
    live LightGBM Quantile Regression model.

    The tool fetches recent OHLCV data from yfinance, engineers all features
    on the fly, loads the trained upper/lower LightGBM models (.txt boosters),
    and runs live inference — no pre-computed lookup tables are used.
    ...
    Returns:
        JSON string with predicted high/low prices, band width %, return
        quantiles, base close used, and model prediction notes.
    """
    # Force LightGBM backend configuration
...
def query_xgboost_price_band(
    ...
    Args:
        ticker: NSE ticker symbol (e.g. 'INFY.NS', 'TCS.NS', 'RELIANCE.NS').
        date: Optional date string 'YYYY-MM-DD'. Defaults to the latest available trading day.
        current_price: Optional override for today's close price (INR).

    Returns:
        JSON string with predicted high/low prices, band width %, return quantiles.
    """
    # Force XGBoost backend configuration
    cfg_xgb = copy.deepcopy(CONFIG)
    cfg_xgb["model"]["backend"] = "xgboost"
...
def _execute_price_band_query(
    ...
    try:
        ...
        logger.exception("Model loading failed for backend '%s'", backend_name)
        return json.dumps({
            "error": f"Failed to load trained {backend_name} models: {exc}",
            "action_required": f"Run training pipeline first: python train_pipeline.py --backend {backend_name}"
        }, indent=2)
    ...
    # 6. Build feature dict for target row
    feat_dict = {c: float(row[c]) for c in feature_cols}
    base_close = float(current_price) if current_price and current_price > 0 else float(row["close"])
    ...
    return json.dumps({
        ...
        "predicts_next_trading_day_band": True,
        "base_close_price_inr": round(base_close, 2),
        f"{backend_name}_predicted_price_band": {
            "predicted_high_price_inr": pred_high_price,
            "predicted_low_price_inr":  pred_low_price,
            "expected_band_width_pct":  f"{band_width_pct}%",
