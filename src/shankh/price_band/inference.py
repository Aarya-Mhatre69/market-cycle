...
# Minimum band half-width as a fraction of close price.
# Prevents the model from producing a nonsensical 0-width band.
_MIN_HALF_WIDTH_FRAC = 0.001   # 0.1 % of close


# ---------------------------------------------------------------------------
# Public: predict price bands
# ---------------------------------------------------------------------------
...
def predict_bands(
    ...
    """
    Run inference and return a DataFrame of price-band predictions.

    Parameters
    ----------
    df : pd.DataFrame
        Feature-engineered frame (output of features.add_features or a
    ...
    -------
    pd.DataFrame
        Original frame with additional columns:
        ``pred_upper_ret``, ``pred_lower_ret``,
        ``pred_high_price``, ``pred_low_price``,
        ``band_width_pct``, ``band_ordering_ok``
    """
    ...
    raw_lower = lower_model.predict(X)

    pred_upper, pred_lower = _enforce_band_ordering(raw_upper, raw_lower, close)
    ...
    n_violations = int((out["band_ordering_ok"] == 0).sum())
    if n_violations:
        # Should never happen after _enforce_band_ordering, but log if it does
        logger.warning("%d band ordering violations after post-processing.", n_violations)

    return out


# ---------------------------------------------------------------------------
# Public: single-row inference (for live/online use)
# ---------------------------------------------------------------------------
