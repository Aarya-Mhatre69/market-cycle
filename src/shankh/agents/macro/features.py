...
"""
Upgraded Feature engineering for stock clustering and forensic anomaly pipeline.
Expands technical, volume, liquidity, tail-risk, and multi-horizon trend features.
"""
...
def build_liquidity_and_volume_features(t_df: pd.DataFrame) -> dict:
    """
    CRITICAL FOR FORENSIC ANOMALY DETECTION:
    Extracts volume surges, illiquidity ratios, and turnover volatility.
    """
    close = t_df["close"].values
    ...
    # 1. Volume Surge Ratio (Short-term Vol / Long-term Vol)
    ...
    # 2. Amihud Illiquidity Measure (|Return| / Turnover)
    # High values = Illiquid stock easily manipulated by small capital
    amihud = float(np.mean(returns / (turnover + 1e-5)) * 1e6)
    
    # 3. Turnover Coefficient of Variation (Volatility of Dollar Volume)
...
def build_cluster_features(df: pd.DataFrame, cfg: Optional[dict] = None) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Extracts multi-dimensional feature profiles per ticker (20 Features).
    """
    logger.info("Extracting expanded multi-factor features for stock clustering...")
    df = df.sort_values(["ticker", "date"]).reset_index(drop=True)

    # Pivot price matrix for correlation distance
    pivoted_prices = df.pivot(index="date", columns="ticker", values="close")

    # Calculate equal-weighted market return benchmark
    df["log_return"] = df.groupby("ticker")["close"].transform(lambda x: np.log(x / x.shift(1)))
    mkt_returns = df.groupby("date")["log_return"].mean().rename("mkt_return")
    df = df.merge(mkt_returns, on="date", how="left")
