"""
Pure ML Model MCP Server: Market Regime HMM Model (Streamable HTTP)
Exposes Gaussian Hidden Markov Model (GaussianHMM) Regime Predictions.
...
logger = logging.getLogger("mcp_market_ml_server")
...
# ============================================================================
# EAGER MODEL CONTAINER: GAUSSIAN HIDDEN MARKOV MODEL (HMM)
# ============================================================================
class MarketRegimeMLContainer:
    """Eagerly loads pre-trained Gaussian HMM model and StandardScaler into RAM."""
    ...
    def __init__(self, models_dir: Path = None):
        ...
        if meta_path.exists():
            with open(meta_path, "r", encoding="utf-8") as f:
                ...
                self.state_mapping = {
                    int(k): str(v) for k, v in raw_mapping.items() if str(k).isdigit()
                }

        if not self.state_mapping:
            self.state_mapping = {i: f"State {i}" for i in range(CONFIG["model"]["n_regimes"])}

        logger.info("Market Regime HMM model eagerly loaded into memory successfully.")
    ...
    def run_inference(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if not validate_features(regime_df):
            raise ValueError("Feature validation failed during HMM inference.")
        ...
        return {
            ...
            "volatility_annualized": round(float(regime_df["mkt_volatility"].iloc[-1]), 2),
            ...
            "historical_regime_states": {
                ...
                for k, v in regime_df[["regime_state", "regime_label"]]
                .tail(10)
...
CONTAINER = MarketRegimeMLContainer()

# ============================================================================
# FAST MCP SERVER INITIALIZATION & PURE MODEL TOOL ENDPOINTS
# ============================================================================
mcp = FastMCP("Market Regime ML Server")
...
def _fetch_universe_ohlcv(tickers: list[str]) -> pd.DataFrame:
    ...
    for tkr in tickers:
        try:
            raw = yf.download(
                tkr, period=f"{_HISTORY_DAYS}d", interval="1d", auto_adjust=False, progress=False
            )
            if raw.empty:
                continue
...
def get_market_regime(query_date: Optional[str] = None) -> str:
    """
    Pure ML Model Inference: Predict current Indian equity market regime using Gaussian HMM.
    ...
    if query_date and query_date.lower() not in ("latest", "today", "now", ""):
        hist = result.get("historical_regime_states", {})
        matching = {k: v for k, v in hist.items() if str(k).startswith(query_date)}
        if matching:
            result["historical_regime_states"] = matching
...
if __name__ == "__main__":
    logger.info("Starting Market Regime Pure ML Model MCP Server on Streamable HTTP (Port 8002)...")
